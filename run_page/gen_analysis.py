...
def generate_basic_statistics(activities, output_dir):
    ...
    for activity_type in available_types:
        print(f"  处理活动类型: {type_manager.type_names.get(activity_type, activity_type)}")
        
        # 周期性统计
        weekly_stats = stats_analyzer.calculate_period_stats(activities, 'weekly', activity_type)
        monthly_stats = stats_analyzer.calculate_period_stats(activities, 'monthly', activity_type)
        yearly_stats = stats_analyzer.calculate_period_stats(activities, 'yearly', activity_type)
        
        # 距离分布
        distance_dist = stats_analyzer.calculate_distance_distribution(activities, activity_type)
...
def generate_advanced_analysis(activities, output_dir):
    ...
    for activity_type in available_types:
        if activity_type in ['running', 'trail_running', 'all']:
            print(f"  分析活动类型: {type_manager.type_names.get(activity_type, activity_type)}")
            
            # 配速趋势分析
            pace_trends = advanced_analyzer.analyze_pace_trends(activities, activity_type)
            
            advanced_stats[activity_type] = {
...
def generate_analysis_overview(activities, output_dir):
    ...
    total_time = sum(type_manager._parse_time(a.get('moving_time', '0')) for a in activities)
    ...
    overview = {
        ...
        'overview': {
            ...
            'avg_distance_per_activity': round(total_distance / total_activities, 2) if total_activities > 0 else 0,
            'avg_time_per_activity': type_manager.format_time(total_time // total_activities) if total_activities > 0 else '0:00'
        },
...
def main():
    ...
    parser.add_argument('--type', choices=['all', 'types', 'basic', 'advanced', 'summary', 'overview'], 
                       default='all', help='生成分析类型')
