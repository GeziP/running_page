...
class AdvancedStatsAnalyzer:
    ...
    def analyze_heart_rate_zones(
        self, activities: List[Dict], max_hr: int = 190, activity_type: str = "all"
    ) -> Dict:
        """
        分析心率区间分布
        ...
        for zone_key, (min_pct, max_pct, zone_name, color) in hr_zones.items():
            ...
            zone_stats[zone_key] = {
                ...
                "percentage": round(
                    (
                        (len(hrs) / total_activities_with_hr * 100)
                        if total_activities_with_hr > 0
                        else 0
                    ),
                    1,
                ),
                "time_percentage": round(
                    (time_in_zone / total_time * 100) if total_time > 0 else 0, 1
                ),
    ...
    def analyze_training_load(
        ...
        weekly_load = defaultdict(
            lambda: {"distance": 0, "time": 0, "activities": 0, "tss": 0}
        )
        monthly_load = defaultdict(
            lambda: {"distance": 0, "time": 0, "activities": 0, "tss": 0}
        )
    ...
    def analyze_performance_metrics(
            ...
            if group_activities:
                ...
                if paces:
                    performance_by_distance[group_name] = {
                        ...
                        "avg_pace": statistics.mean(paces),
                        "worst_pace": max(paces),
                        "best_pace_formatted": self.type_manager.format_time(
                            int(min(paces))
                        ),
                        "avg_pace_formatted": self.type_manager.format_time(
                            int(statistics.mean(paces))
                        ),
                        "pace_improvement": self._calculate_pace_improvement(
                            paces, group_activities
                        ),
        ...
        return {
            ...
            "overall_trends": self._analyze_overall_performance_trends(
                filtered_activities
            ),
        }
    ...
    def _generate_pace_insights(
        self, pace_data: List[Dict], monthly_trends: Dict
    ) -> List[str]:
        """生成配速洞察"""
        insights = []
        ...
        if len(monthly_trends) >= 2:
            ...
            improvements = [
                monthly_trends[m]["improvement"]
                for m in recent_months
                if monthly_trends[m]["improvement"] != 0
            ]
    ...
    def _generate_hr_insights(
        self, zone_stats: Dict, monthly_trends: Dict
    ) -> List[str]:
        """生成心率洞察"""
        insights = []
        ...
        if total_activities > 0:
            zone2_3_pct = (
                (zone_stats["zone2"]["count"] + zone_stats["zone3"]["count"])
                / total_activities
                * 100
            )
            if zone2_3_pct > 70:
            ...
            zone4_5_pct = (
                (zone_stats["zone4"]["count"] + zone_stats["zone5"]["count"])
                / total_activities
                * 100
            )
            if zone4_5_pct > 30:
    ...
    def _calculate_pace_improvement(
        self, paces: List[float], activities: List[Dict]
    ) -> float:
        """计算配速改进幅度"""
        if len(paces) < 2:
            return 0
        ...
        sorted_activities = sorted(
            activities, key=lambda x: x.get("start_date_local", "")
        )
        first_pace = None
        last_pace = None
