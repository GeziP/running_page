...
class BasicStatsAnalyzer:
    ...
    def calculate_period_stats(self, activities: List[Dict], period_type: str = 'monthly', activity_type: str = 'all') -> Dict:
        """
        计算周期性统计数据
    ...
    def calculate_distance_distribution(self, activities: List[Dict], activity_type: str = 'all') -> Dict:
        """
        计算距离分布统计
        ...
        for min_dist, max_dist, label in distance_ranges:
            ...
            distribution[label] = {
                ...
                'percentage': round(count / len(filtered_activities) * 100, 1) if filtered_activities else 0
            }
    ...
    def calculate_weekly_summary(self, activities: List[Dict], activity_type: str = 'all') -> Dict:
        """
        计算最近周统计摘要
        ...
        # 按活动类型筛选
        filtered_activities = self.type_manager.filter_by_type(recent_activities, activity_type)
    ...
    def calculate_monthly_summary(self, activities: List[Dict], activity_type: str = 'all') -> Dict:
        """
        计算当月统计摘要
        ...
        # 按活动类型筛选
        filtered_activities = self.type_manager.filter_by_type(monthly_activities, activity_type)
        ...
        return {
            ...
            'stats': self._calculate_period_summary(filtered_activities),
            'weekly_breakdown': self._calculate_weekly_breakdown_in_month(filtered_activities, month_start)
        }
    
    def calculate_yearly_summary(self, activities: List[Dict], year: int = None, activity_type: str = 'all') -> Dict:
        """
        计算年度统计摘要
        ...
        # 按活动类型筛选
        filtered_activities = self.type_manager.filter_by_type(yearly_activities, activity_type)
        ...
        return {
            ...
            'year': year,
            'stats': self._calculate_period_summary(filtered_activities),
            'monthly_breakdown': self._calculate_monthly_breakdown_in_year(filtered_activities, year)
        }
    
    def get_personal_bests(self, activities: List[Dict], activity_type: str = 'all') -> Dict:
        """
        获取个人最佳记录
        ...
        for activity in filtered_activities:
            ...
            # 最快配速（仅跑步类活动）
            if activity.get('type') in ['Run', 'TrailRun'] and distance_km > 1:  # 至少1公里才计算配速
    ...
    def _calculate_period_summary(self, activities: List[Dict]) -> Dict:
        ...
        total_distance = sum(float(a.get('distance', 0)) for a in activities) / 1000  # 转换为公里
        total_time = sum(self.type_manager._parse_time(a.get('moving_time', '0')) for a in activities)
        ...
        avg_pace = self.type_manager.format_pace(total_distance, total_time) if total_distance > 0 else '0:00'
    ...
    def _calculate_overall_summary(self, period_stats: Dict) -> Dict:
        ...
        return {
            ...
            'total_distance': round(total_distance, 2),
            'avg_activities_per_period': round(total_activities / len(periods), 1) if periods else 0,
            'avg_distance_per_period': round(total_distance / len(periods), 2) if periods else 0
        }
    ...
    def _calculate_weekly_breakdown_in_month(self, activities: List[Dict], month_start: datetime) -> Dict:
        """计算月内周分解统计"""
    ...
    def _calculate_monthly_breakdown_in_year(self, activities: List[Dict], year: int) -> Dict:
        """计算年内月分解统计"""
