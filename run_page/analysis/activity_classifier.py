...
class ActivityTypeManager:
    ...
    def filter_by_type(self, activities: List[Dict], activity_type: str = 'all') -> List[Dict]:
        """
        根据活动类型筛选活动
        ...
        return [activity for activity in activities if activity.get('type') in allowed_types]
    
    def filter_by_multiple_types(self, activities: List[Dict], activity_types: List[str]) -> List[Dict]:
        """
        根据多个活动类型筛选活动（用于对比模式）
    ...
    def get_type_statistics(self, activities: List[Dict]) -> Dict[str, Dict]:
        ...
        for activity_type, type_activities in grouped_activities.items():
            ...
            total_distance = sum(float(a.get('distance', 0)) for a in type_activities) / 1000  # 转换为公里
            total_time = sum(self._parse_time(a.get('moving_time', '0:00:00')) for a in type_activities)
            ...
            stats[activity_type] = {
                ...
                'avg_time': avg_time,
                'percentage': round(count / len(activities) * 100, 1) if activities else 0
            }
