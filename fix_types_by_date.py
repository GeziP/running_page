...
def is_likely_cycling(distance_km, speed_kmh, pace_min_per_km, duration_hours):
    ...
    if distance_km > 40 and pace_min_per_km < 3.5:
        return (
            True,
            f"长距离({distance_km:.1f}km) + 快配速({pace_min_per_km:.1f}min/km)",
        )
...
def analyze_unmatched_activities():
    ...
    for activity in run_activities:
        ...
        if len(time_parts) == 3:  # H:MM:SS
            duration_hours = (
                int(time_parts[0]) + int(time_parts[1]) / 60 + int(time_parts[2]) / 3600
            )
            total_minutes = (
                int(time_parts[0]) * 60 + int(time_parts[1]) + int(time_parts[2]) / 60
            )
        ...
        is_cycling, reason = is_likely_cycling(
            distance_km, speed_kmh, pace_min_per_km, duration_hours
        )
